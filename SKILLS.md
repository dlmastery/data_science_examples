...
| **14** | [`14_autogluon_multimodal_automl_suite`](./14_autogluon_multimodal_automl_suite) | `automl-autogluon`<br>`timeseries-chronos`<br>`multimodal-fusion`<br>`shap-interpretability-guide`<br>`model-distillation`<br>`mlops-model-monitoring`<br>`crisp-dm-audit-framework` | 3-Level Stacking DAG, Caruana forward selection, Chronos T5 Transformer foundation probabilistic time series, Vision-Language-Tabular deep fusion, 9μs student distillation, and 10-page KaTeX paper. |
| **15** | [`15_spy_timeseries_sota_forecasting`](./15_spy_timeseries_sota_forecasting) | `spy-timeseries-forecasting`<br>`timeseries-chronos`<br>`attention-transformers`<br>`stacking-ensembles`<br>`financial-econometrics`<br>`shap-interpretability-guide`<br>`crisp-dm-audit-framework` | Next-day & next-week SPY target price forecasting with Chronos-T5, PatchTST, TFT, 2-Level Stacking DAG, Purged & Embargoed walk-forward backtesting (Sharpe 2.15, Sortino 2.80), and AST zero-leakage code auditor. |
